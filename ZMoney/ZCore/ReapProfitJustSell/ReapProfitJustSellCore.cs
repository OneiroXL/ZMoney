    ...
    public class ReapProfitJustSellCore
    {
        ...
        public static void SPOTBinance(string symbol, bool isDebug = true) 
        {
            ...
            while (true) 
            {
                ...
                //计算价格和数量
                decimal quantity = Math.Ceiling(BinanceUSDTConstant.MinTradingVolume / querySymbolNewestPriceRModel.Price);
                ...
                if (buySpotTradeOrderRModel.Status == SPOTTradestatusEnum.FILLED)
                {
                    ...
                    while (true)
                    {
                        ...
                        //当获利时
                        if (querySymbolNewestPriceRModel.Price - buySpotTradeOrderPModel.Price > 0.015m)
                        {
                            //现货下单(卖出)
